...
async function submit() {
  ...
  loading.value = true
  try {
    const res  = await fetch(`${window.__APP_CONFIG__.apiBase}/login`, {
      method:  'POST',
    ...
    if (!res.ok) {
      ...
      else if (res.status === 401 && data.errors) Object.assign(errors, data.errors)
  ...
  <div class="reg-page">
    ...
    <div class="reg-card">
      <div class="reg-card__head">
        <span class="reg-card__label">Admin / Login</span>
        <h1 class="reg-card__title">Вход</h1>
      ...
      <form class="reg-form" @submit.prevent="submit" novalidate>
        ...
        <button class="reg-btn" type="submit" :disabled="loading">
          <span class="reg-btn__text">{{ loading ? 'Вход…' : 'Войти' }}</span>
      ...
      <p class="reg-card__foot">
        Нет аккаунта?
        <RouterLink class="reg-card__link" to="/admin/register">Зарегистрироваться</RouterLink>
      </p>
    </div>
  </div>
</template>
